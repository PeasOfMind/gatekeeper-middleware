...
function gateKeeper(req, res, next) {
  ...
  const userEntry = req.get('x-username-and-password');
  const userObj = queryString.parse(userEntry);
  const matchedUser = USERS.find(user => {
    if (user.userName === userObj.user && user.password === userObj.pass) return user;
  });
